...
var _aegLogger = require('@adexchange/aeg-logger');

var _aegLogger2 = _interopRequireDefault(_aegLogger);
...
var conf = _config2.default.get('aeg-limelight-api');

var Api = function () {
	function Api(apiConfig) {
		_classCallCheck(this, Api);

		_lodash2.default.extend(this, apiConfig);
	...
	_createClass(Api, [{
		key: 'findActiveCampaigns',
		value: function findActiveCampaigns(callback) {
			this.apiRequest('membership', 'campaign_find_active', {}, callback);
		}
	}, {
		key: 'getCampaign',
		value: function getCampaign(campaignId, callback) {
		...
		key: 'findOrders',
		value: function findOrders(params, callback) {
			...
			if (params.product_ids && !_lodash2.default.isString(params.product_ids)) {
		...
		key: 'findUpdatedOrders',
		value: function findUpdatedOrders(params, callback) {
			...
			if (!_lodash2.default.isString(params.group_keys)) {
		...
		key: 'updateOrders',
		value: function updateOrders(params, callback) {
			...
			if (!_lodash2.default.isString(params.orderIds)) {
				params.orderIds = params.orderIds.join(',');
			}

			if (!_lodash2.default.isString(params.actions)) {
				params.actions = params.actions.join(',');
			}

			if (!_lodash2.default.isString(params.values)) {
				params.values = params.values.join(',');
			}

			this.apiRequest('membership', 'order_update', {
		...
		key: 'getCustomer',
		value: function getCustomer(customerId, callback) {
		...
		key: 'findCustomers',
		value: function findCustomers(params, callback) {
		...
		key: 'getProducts',
		value: function getProducts(productIds, callback) {

			if (!productIds || productIds.length === 0) {
				return callback({ responseCode: 500, responseCodeDesc: 'getProducts must product ids' });
			}

			if (!_lodash2.default.isString(productIds)) {
		...
		key: 'findShippingMethods',
		value: function findShippingMethods(params, callback) {
		...
		key: 'composeApiCall',
		value: function composeApiCall(apiType, method, params) {
			...
			if (params !== null) {
				_lodash2.default.extend(form, params);
			}
		...
		key: 'apiRequest',
		value: function apiRequest(apiType, method, params, callback) {

			var requestParams = this.composeApiCall(apiType, method, params);

			var self = this;

			_request2.default.post(requestParams, function (err, response, body) {
				...
				if (body.response_code) {
					...
					_aegLogger2.default.error(body);
				...
				if (result.body && result.body.data) {
					try {
						...
						_aegLogger2.default.error('Failed to parse result.body.data', ex);
						_aegLogger2.default.error(requestParams);
						_aegLogger2.default.error(result.body.data);
						return callback(ex);
					}
				}
				...
				_aegLogger2.default.error('API request error', { apiType: apiType, method: method, err: err });
			});
		}
	}]);
