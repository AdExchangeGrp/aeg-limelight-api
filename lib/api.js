...
Api.prototype.getCampaign = function (campaignId, callback) {
	...
	if (!campaignId) {
		callback({responseCode: 500, responseCodeDesc: 'getCampaign must have a campaign id'});
		return;
	}
...
Api.prototype.getOrder = function (orderId, callback) {
	...
	if (!orderId) {
		callback({responseCode: 500, responseCodeDesc: 'getOrder must have an order id'});
		return;
	}
...
Api.prototype.getOrders = function (orderIds, callback) {
	...
	if (!orderIds || orderIds.length === 0) {
		callback({responseCode: 500, responseCodeDesc: 'getOrders must have order ids'});
		return;
	}
...
Api.prototype.findOrders = function (params, callback) {
	...
	if (!params || !params.product_ids ||
		params.product_ids.length === 0 || !params.campaign_id || !params.criteria || !params.start_date || !params.end_date) {
		callback({
			responseCode: 500,
			responseCodeDesc: 'findOrders requires params: campaign_id, product_ids, criteria, start_date, end_date'
		});
		return;
	}

	if (!_.isString(params.product_ids)) {
...
Api.prototype.getCustomer = function (customerId, callback) {
	...
	if (!customerId) {
		callback({responseCode: 500, responseCodeDesc: 'getCustomer must have a customer id'});
		return;
	}
...
Api.prototype.getProducts = function (productIds, callback) {
	...
	if (!productIds || productIds.length === 0) {
		callback({responseCode: 500, responseCodeDesc: 'getProducts must product ids'});
		return;
	}
	...
	this.apiRequest('membership', 'product_index', productIds, callback);
};
